...
% VERSIÓN REVISADA — respuesta a los revisores LISA 2026 (limpia). Original: paper.tex
...
\textbf{0.839}; a per-plane analysis and a five-way slice-sampling
ablation quantify how unevenly a fixed three-slice input represents spatially
localised artifacts across acquisition orientations. For \textbf{Task~1B} (low-field image-quality enhancement) we
...
$0.5$--$0.6$ correlation that leaves the supervisory target only partially
aligned, and a
metric-overfitting (Goodhart) effect in checkpoint selection. For
\textbf{Task~2} (3D segmentation of eleven annotated subcortical labels) a
residual-encoder nnU-Net trained on the full cohort with stock components attains
a mean Dice of \textbf{0.784} over the eleven annotated labels
...
for 3D segmentation of subcortical gray matter over eleven annotated labels.
...
evaluation, validation and curation right. We state the trade-off
...
\begin{enumerate}
  ...
  \item \textbf{(Task~2)} A no-frills residual-encoder nnU-Net that reaches mean
        Dice 0.784 over the eleven annotated labels (0.785 over the nine
...
volumetric evidence. This buys volumetric coverage at the fixed cost of the
...
across folds. The submitted predictor averages the sigmoid outputs of all ten models
...
folding it into the headline number. Per-class thresholds
...
the corpus callosum. The ranking aggregates five metrics --- $1-$DSC, HD, HD95, ASSD (mm)
...
\begin{table}[htbp]
\caption{Task~1A configuration ablation on OOF predictions (micro-accuracy over
...
ordinal distance, is highest for Noise (0.81) and Motion (0.71) and lowest for
...
so at $\approx0.5$ alignment the model synthesises high-field texture at
slightly displaced locations, which the reference metrics penalise. Our selection guard correctly rejected all M4
checkpoints.
...
ensemble over all 79 cases, reported over all eleven annotated labels with the
...
Table~\ref{tab:perstruct} gives the per-structure breakdown, over all eleven
...
(Dice 0.876/0.875). Caudate and lentiform form a consistent middle band around
...
and scored under the 2026 criteria). Thalamus and lentiform are recovered closely; the hippocampus
...
makes pixel and feature losses reward texture which is plausible but
...
nnU-Net's per-label selection. The residual errors are physically interpretable throughout: the
...
label-free threshold transport. All stay within the paper's premise: at 0.064\,T the gains come from
